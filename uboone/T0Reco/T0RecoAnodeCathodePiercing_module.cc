...
// Initialize member data here.
{
  ...
  fDTMin       = p.get<double>     ("DTMin");
  fDTMax       = p.get<double>     ("DTMax");
...
void T0RecoAnodeCathodePiercing::produce(art::Event & e)
{
  ...
  for (auto& track : TrkVec){
    ...
    if( flash_match_result.first < fDTMin || flash_match_result.first > fDTMax )
      continue;
    ...
    util::CreateAssn(*this, e, *T0_v, flash_ptr, *flash_t0_assn_v);
...
std::pair<double,size_t> T0RecoAnodeCathodePiercing::FlashMatch(const double reco_time){
  ...
  for (size_t i=0; i < _flash_times.size(); i++){
    auto const& time = _flash_times[i] - 3.0;
    double dt = time - reco_time;
    if (fabs(dt) < dt_min){
      dt_min  = fabs(dt);
