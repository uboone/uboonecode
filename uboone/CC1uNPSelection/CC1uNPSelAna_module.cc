...
public:
    ...
    int Topology(int nmuons, int nelectrons, int npions, int npi0, int nprotons, int cosmicflag, int OOFVflag);
...
void  CC1uNPSelAna::beginJob()
{
    ...
    fMC_ntrks->Branch("fNRecoTrks", &fNRecoTrks, "fNRecoTrks/I");
    ...
    fMC_noshwr->Branch("fNRecoTrks", &fNRecoTrks, "fNRecoTrks/I");
    ...
    fMC_trkfls->Branch("fNRecoTrks", &fNRecoTrks, "fNRecoTrks/I");
    ...
    fMC_mupinFV->Branch("fLlep", &fLlep, "fPLep/F");
    ...
    fMC_TrunMean->Branch("fLlep", &fLlep, "fPLep/F");
...
void  CC1uNPSelAna::reconfigure(fhicl::ParameterSet const& pset)
{
    ...
    fPandoraNuVertexModuleLabel  = pset.get< std::vector<std::string> >("PandoraNuVertexModuleLabel", std::vector<std::string>()={"pandoraNu"}); 
    ...
    fBeamMin                 = pset.get      ("BeamMin", 3.2);    
    fBeamMax                 = pset.get      ("BeamMax", 4.8);    
...
{		
  //get all the reconstructed hits and try to match the true hits and find out the true particles
  //
...
int CC1uNPSelAna::Topology(int nmuons, int nelectrons, int npions, int npi0, int nprotons, int cosmicflag, int OOFVflag)
{
...
void  CC1uNPSelAna::analyze(const art::Event& event)
{
    ...
    */
    ...
    _fTruenuvrtxx_SCE=  mctruth->GetNeutrino().Nu().Vx() - SCE->GetPosOffsets(geo::Point_t(mctruth->GetNeutrino().Nu().Vx(),mctruth->GetNeutrino().Nu().Vy(),mctruth->GetNeutrino().Nu().Vz())).X(); 
    _fTruenuvrtxy_SCE=  mctruth->GetNeutrino().Nu().Vy() + SCE->GetPosOffsets(geo::Point_t(mctruth->GetNeutrino().Nu().Vx(),mctruth->GetNeutrino().Nu().Vy(),mctruth->GetNeutrino().Nu().Vz())).Y();
    _fTruenuvrtxz_SCE = mctruth->GetNeutrino().Nu().Vz() + SCE->GetPosOffsets(geo::Point_t(mctruth->GetNeutrino().Nu().Vx(),mctruth->GetNeutrino().Nu().Vy(),mctruth->GetNeutrino().Nu().Vz())).Z();
    ...
    std::cout<<"OOFVflag= "<<OOFVflag<<std::endl;
    ...
    for(size_t g4pt=0; g4pt<mcparticleListHandle->size(); g4pt++){
      ...
      _fg4SCEcorrStartPointx.push_back(pPart.Vx() - SCE->GetPosOffsets(geo::Point_t(pPart.Vx(),pPart.Vy(),pPart.Vz())).X());
      _fg4SCEcorrStartPointy.push_back(pPart.Vy() + SCE->GetPosOffsets(geo::Point_t(pPart.Vx(),pPart.Vy(),pPart.Vz())).Y());
      _fg4SCEcorrStartPointz.push_back(pPart.Vz() + SCE->GetPosOffsets(geo::Point_t(pPart.Vx(),pPart.Vy(),pPart.Vz())).Z());



      _fg4SCEcorrEndPointx.push_back(pPart.EndPosition()[0] - SCE->GetPosOffsets(geo::Point_t(pPart.EndPosition()[0],pPart.EndPosition()[1],pPart.EndPosition()[2])).X());
      _fg4SCEcorrEndPointy.push_back(pPart.EndPosition()[1] + SCE->GetPosOffsets(geo::Point_t(pPart.EndPosition()[0],pPart.EndPosition()[1],pPart.EndPosition()[2])).Y());
      _fg4SCEcorrEndPointz.push_back(pPart.EndPosition()[2] + SCE->GetPosOffsets(geo::Point_t(pPart.EndPosition()[0],pPart.EndPosition()[1],pPart.EndPosition()[2])).Z());
      ...
      //_fg4origin.push_back(pPart.origin());
      _fg4origin.push_back(mc_truth->Origin());  //what created particle
      
      
      //_fg4pathlen[geant_particle]  = plen;
      //_fg4pathlen_drifted[geant_particle]  = plendrifted;

      _fg4MCTruthIndex.push_back(mc_truth.key());

      _fg4NumberDaughters.push_back(pPart.NumberDaughters());
      //_fg4inTPCActive[geant_particle] = int(isActive);
      //_fg4inTPCDrifted[geant_particle] = int(isDrifted);
   ...
   if(nmuons==0 && nelectrons==0 && npions==0 && npi0==0 && nprotons==0 && OOFVflag==false) {cosmicflag=true;} 

   Int_t TopFlag=Topology(nmuons, nelectrons, npions, npi0, nprotons, cosmicflag, OOFVflag);


   std::cout<<"Topology flag is: "<<TopFlag<<std::endl;
    //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    lar_pandora::LArPandoraHelper::CollectVertices(event, fPandoraNuVertexModuleLabel[0], vertexVector, particlesToVertices);  
	    ...
	    //imcshwr != showerHandle->end(); ++imcshwr) {
            ...
            */
            ...
            for(size_t shwrIdx=0; shwrIdx<  showerHandle->size(); shwrIdx++){
                  ...
                  double vershwrdist=(shwrPos-temp).Mag();
