...
public:
    ...
    TFile *frecombination;
    TProfile* fdedx_range_pro;
    TProfile* fdedx_range_ka;
    TProfile* fdedx_range_pi;
    TProfile* fdedx_range_mu;
    // ---- - - -- -- - -- -- -- -- --- - - - - -- --- - - - --- -- - -
    // debug
    // ---- - - -- -- - -- -- -- -- --- - - - - -- --- - - - --- -- - -
    Int_t debug=5;
            ...
            for (auto const& calo : calos){
                   if (calo->PlaneID().isValid){
                       ...
                       for (size_t ip = 0; ip < calo->dEdx().size(); ++ip){
                           ...
                           //track.SetPandoraNuNewCaliPID(plane, frecombination);
                       }
                       if (used_trkres) pida /= used_trkres;
                       track.SetPIDaPerPlane( plane , pida );
                       track.SetPandoraNuNewCaliPID( plane, fdedx_range_pro, fdedx_range_ka, fdedx_range_pi, fdedx_range_mu );
                   //}
               }
               track.SetPIDa();
...
void ub::ErezCCQEAna::FilterGoodPairVertices(){
    ...
    for (auto & v:tmp_vertices) {
        ...
        if (
            ...
            //auto Track_muCandidate = v.GetLongestTrack();
	    std::cout<<"inside FilterGoodPairVertices Track_muCandidate Chi2 "<<Track_muCandidate.GetCaliPID_Chi2Proton(2)<<std::endl;
	    std::cout<<"inside FilterGoodPairVertices Track_muCandidate NewChi2 "<<Track_muCandidate.GetNewCaliPID_Chi2Proton(2)<<std::endl;
            ...
            //auto Track_pCandidate = v.GetShortestTrack();
	    std::cout<<"inside FilterGoodPairVertices Track_pCandidate Chi2 "<<Track_pCandidate.GetCaliPID_Chi2Proton(2)<<std::endl;
	    std::cout<<"inside FilterGoodPairVertices Track_pCandidate NewChi2 "<<Track_pCandidate.GetNewCaliPID_Chi2Proton(2)<<std::endl;
            auto PpFromRange = trkm.GetTrackMomentum( Track_pCandidate.GetLength() , 2212);
            v.AssignProtonTrack( Track_pCandidate );
            
	    //if (Track_pCandidate.GetCaliPID_Chi2Proton(2) > Track_muCandidate.GetCaliPID_Chi2Proton(2) ) std::cout<<"inside FilterGoodPairVertices STOP muchi2<pchi2"<<std::endl;
	    if (Track_muCandidate.GetLength() < Track_pCandidate.GetLength()) std::cout<<"inside FilterGoodPairVertices STOPSTOP wrong length"<<std::endl; 
...
void ub::ErezCCQEAna::HeaderVerticesInCSV(){
    ...
    << "pidcali_NewChi2ProtonYplane_muCandidate"   << ","
    << "pidcali_NewChi2ProtonYplane_pCandidate"    << ","
...
void ub::ErezCCQEAna::StreamVerticesToCSV(){
    ...
    for (auto v:vertices){
        ...
        << trk_mu.GetNewCaliPID_Chi2Proton(2)    << ","
        << trk_p.GetNewCaliPID_Chi2Proton(2)     << ","
...
void ub::ErezCCQEAna::endJob(){
    ...
    summary_file.close();
    std::cout<<"When filling tot pot is : "<<pot_total<<std::endl; 
...
void ub::ErezCCQEAna::endSubRun(const art::SubRun& sr){
    ...
    if ((fPOTModuleLabel=="generator") && (sr.getByLabel(fPOTModuleLabel,potListHandle))) // for MC-BNB
    {
       std::cout<<"ErezCCQEAna::endSubRun fPOTModuleLabel "<<fPOTModuleLabel<<" pot "<<pot<<" potListHandle->totpot "<<potListHandle->totpot<<" pot_total "<<pot_total<<std::endl;
        pot = potListHandle->totpot - pot_total; // AA AP adding the POT bug fix 
    } 
    else if ((fPOTModuleLabel=="beamdata") && (sr.getByLabel(fPOTModuleLabel,"bnbETOR860",potListHandle))) // for BNB, Marco Del Tutto, Aug-27, 2017
    {
    ...
    pot_total += pot;
    std::cout<<" The total pot is "<<pot_total<<std::endl; 
...
void ub::ErezCCQEAna::reconfigure(fhicl::ParameterSet const & p){
    ...
    sp.find_file("CCQE/dEdxrestemplates.root",fname); 
    std::cout<<"fname"<<fname;
    //TFile infile(fname.c_str(), "READ");
    frecombination = new TFile(fname.c_str(), "READ");
    //if(!frecombination->IsOpen()) throw cet::exception("CCQEAna") << "Could not find the recombination effect file '" << fname << "'!\n";
    if(!frecombination->IsOpen()) std::cout<< "Could not find the recombination effect file '" << fname << "'!\n";
    fdedx_range_pro = (TProfile*) frecombination->Get("dedx_range_pro");
    fdedx_range_ka = (TProfile*) frecombination->Get("dedx_range_ka");
    fdedx_range_pi = (TProfile*) frecombination->Get("dedx_range_pi");
    fdedx_range_mu = (TProfile*) frecombination->Get("dedx_range_mu");
    frecombination->Close();
    delete frecombination;
}
