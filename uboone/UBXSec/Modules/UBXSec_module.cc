  ...
  for (unsigned int slice = 0; slice < tpcobj_h->size(); slice++){
    ...
    */
    ...
    if (muon_cand_exists) {
      ...
      std::cout << "[UBXSec] Gap of " << end_wire - start_wire << ", n of dead wires in it: " << n_dead_wires << std::endl;
      double r = _track_quality.GetR();
      std::cout << "[UBXSec] The r value is: " << r << std::endl;
      ...
      std::cout << "[UBXSec] Percentage of used hit in cluster = " << ratio << std::endl;
  ...
  if (_is_mc) {
    ...
    if (softwareTriggerHandle.isValid()){ 
      if (softwareTriggerHandle->getNumberOfAlgorithms() == 1) {
        ...
        std::cout << "[UBXSec] SW trigger name: " << algoNames[0] << std::endl;
    ...
    if( !nuMcflash_h.isValid() || nuMcflash_h->empty() ) {
      std::cerr << "[UBXSec] Don't have neutrino MC flashes." << std::endl;
      //return;
    } else {
    ...
    if (nuMcflash_h->size() > 0) {
      std::cout << "[UBXSec] We have a neutrino MCFlash, and its time is: " << (*nuMcflash_h)[0].Time() << std::endl;
    } else if (nuMcflash_h->size() == 0) {
      if(opActivityInBeamSpill) {
        std::cout << "[UBXSec] No MCFlash but optical activity in the beam spill." << std::endl;
