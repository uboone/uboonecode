...
namespace lee {
  ...
  std::pair<std::vector<double>, size_t> EnergyHelper::showerEnergyV(const art::Ptr<recob::Shower> &shower,
								     const art::Event &evt,
								     std::string _pfp_producer) {
    ...
    std::vector<double> energy = {0, 0, 0};
    int n_hits[3] = {0, 0, 0};
    for (size_t icl = 0; icl < clusters.size(); icl++) {
      art::FindManyP<recob::Hit> hit_cluster_ass(cluster_handle, evt,
						 _pfp_producer);
    ...
    for(size_t i = 0; i < 3; ++i) energy[i] *= 1e-3;

    return std::make_pair(energy, std::distance(n_hits, std::max_element(n_hits, n_hits + n)));
  ...
  void EnergyHelper::dQdx(size_t pfp_id,
    ...
    geoHelper.correct_direction(pfp_id, evt, _pfp_producer);
