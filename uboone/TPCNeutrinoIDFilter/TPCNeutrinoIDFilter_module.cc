   ...
   // if (!pass)
    //{
        ...
        if (cosmicVecHandle.isValid())
        {
            ...
            mf::LogInfo("TPCNeutrinoIDFilter") << "cosmicClusterAssns.size: "<<cosmicClusterAssns.size()<<std::endl;
            if (cosmicClusterAssns.isValid() && cosmicClusterAssns.size() > 0 /*&& cosmicClusterAssns.at(1).size() > 0*/)
            {
                pass = true;
            }
        }
