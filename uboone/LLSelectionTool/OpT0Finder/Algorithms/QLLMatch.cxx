...
QLLMatch *_qll_global = nullptr; // A global pointer to be used for MINUIT
                     ...
                     const Flash_t &measurement)
{
  ...
  for (size_t pmt_index = 0; pmt_index < hypothesis.pe_v.size(); ++pmt_index)
  {
    ...
    if (O < 0)
    {
      if (H < _penalty_threshold_v[pmt_index])
        continue;
                 ...
                 Int_t /*Flag*/)
{
  ...
  auto const &hypothesis = _qll_global->ChargeHypothesis(*Xval);
  auto const &measurement = _qll_global->Measurement();
  Fval = _qll_global->QLL(hypothesis, measurement);

  _qll_global->Record(Xval[0]);
