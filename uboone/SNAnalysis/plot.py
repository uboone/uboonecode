from ROOT import TFile, TH1F, TCanvas, gDirectory, gROOT, gPad, TLegend, TPad, TLine
import math

gROOT.ForceStyle()

# ROOT files with histograms
snfile = TFile("/home/jcrespo/MicroBooNE/SNAnalysis/19021/spectra/SNRun19021_960files3planes_SNMichelAna_hist_v2.root")
extunbfile = TFile("/home/jcrespo/MicroBooNE/SNAnalysis/EXTUNB/spectra/Run3_25kfiles3planes_SNMichelAna_hist_v2.root")
...
for rootdir in snfile.GetListOfKeys():
    ...
    for hname in hlistname:
        ...
        if htemp.GetName().startswith("sn_hEventHitMult"):
            ...
            normfactor[int( rootdir.GetName()[-1] )] = htemp.GetEntries()
...
for rootdir in extunbfile.GetListOfKeys():
    ...
    for hname in hlistname:
        ...
        if htemp.GetName().startswith("extunb_hEventHitMult"):
            ...
            normfactor[int( rootdir.GetName()[-1] )] *= 1./(2.*htemp.GetEntries())
...
for snh in snhlist:
    ...
    if extunbh.GetName().startswith("extunb_hEventHitMult"):
        ...
        extunbh.Scale( 2.*normfactor[int( snh.GetName()[-1] )] )
    else:
