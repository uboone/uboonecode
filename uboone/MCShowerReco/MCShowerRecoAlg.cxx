...
namespace larreco {
  ...
  void MCShowerRecoAlg::RecoMCShower(const art::Event &evt)
  {
	...
	if(magnitude < min_magnitude) {
	  fShowerProf.at(shower_index).vtxDaughter.at(0) = vtx.at(0);
	  fShowerProf.at(shower_index).vtxDaughter.at(1) = vtx.at(1);
	  fShowerProf.at(shower_index).vtxDaughter.at(2) = vtx.at(2);
	  min_magnitude = magnitude; 

	  double min_time=1e9;
	  for(auto hit_iter = edep.mchits.begin();
	      hit_iter != edep.mchits.end();
	      ++hit_iter) {
	    
	    if(min_time > (*hit_iter).second.timeStart)
	      min_time = (*hit_iter).second.timeStart;
	  }

	  fShowerProf.at(shower_index).vtxDaughter.at(3)=min_time;
	}
	...
	std::cout
	  ...
	  << std::endl
	  << Form("    ... with %zu daughters start @ (%g,%g,%g,%g) sum momentum (%g,%g,%g,%g)",
		  prof.daughterTrackId.size(),
		  prof.vtxDaughter.at(0),prof.vtxDaughter.at(1),prof.vtxDaughter.at(2),prof.vtxDaughter.at(3),
